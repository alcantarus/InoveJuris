...
export default function FinanceiroPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const contractData = {
      ...
      gps_paid_value: formData.hasGpsControl ? (formData.gps_paid_value || null) : null,
    ...
    if (editingContract) {
      ...
      const { data: allExisting, error: fetchErr } = await supabase.from('installments')
        .select('id, amountPaid, installmentNumber')
        .eq('contract_id', editingContract.id);
      console.log('All existing installments (for debug):', allExisting);
  ...
  const handleCancelClick = async (e: React.MouseEvent, contract: Contract) => {
    ...
    const allowedStatuses = ['aberto', 'atrasada', 'prorrogada', 'prorrogado']
    const hasNonCancellableInstallments = insts?.some((i: Installment) => !allowedStatuses.includes(i.status?.toLowerCase()) || i.amountPaid > 0)
